		...
		public /*.void.*/ function rpr_options_submenu() {
			...
			<form method="post">
					...
					<tbody class="fields">
						...
						if ( is_array( $redux_usermeta ) ) {
							foreach ( $redux_usermeta as $index => $meta_field ) {
								...
								echo "\n", '<td><input type="text" name="options[', $index, ']" id="options[', $index, ']" value="', esc_attr( $meta_field['options'] ), '"';
								if ( 'textbox' !== $meta_field['display'] && 'select' !== $meta_field['display'] && 'checkbox' !== $meta_field['display'] && 'radio' !== $meta_field['display'] ) { echo ' readonly="readonly"';
								...
								echo "\n", '<td><input type="text" name="terms_agreement_text[', $index, ']" id="terms_agreement_text[', $index, ']" value="', esc_attr( $meta_field['terms_agreement_text'] ), '"';
								if ( 'terms' !== $meta_field['display'] ) { echo ' readonly="readonly"';
								} echo ' style="width: 100%;" /></td></tr>';

								echo "\n", '<tr><td>', __( 'Revised', 'register-plus-redux' ), '</td>';
								echo "\n", '<td><input type="text" name="date_revised[', $index, ']" id="date_revised[', $index, ']" class="datepicker" value="', date( 'm/d/Y', $meta_field['date_revised'] ), '"';
								if ( 'terms' !== $meta_field['display'] ) { echo ' readonly="readonly"';
								} echo ' style="width: 100%;" /></td></tr>';
				...
				*/ ?>
				...
				<p><code>user_login user_email first_name last_name user_url aim yahoo jabber description invitation_code<?php if ( is_array( $redux_usermeta ) ) { foreach ( $redux_usermeta as $meta_field ) { echo ' ', $meta_field['meta_key'];}
} ?></code></p>
		...
		public /*.void.*/ function rpr_options_submenu_footer() {
			...
			function updateAdminMessageSummary() {
				...
				if (jQuery("#disable_admin_message_registered").prop("checked") && jQuery("#disable_admin_message_created").prop("checked")) {
					...
					adminMessage.append(jQuery("<div>").text("<?php _e( 'To: ', 'register-plus-redux' );
							...
							foreach ( $user_query->results as $user ) {
								...
								<tr id="user-<?php echo $user->ID; ?>"<?php echo $style; ?>>
									...
									<td class="username column-username">
										<strong><?php if ( current_user_can( 'edit_users' ) ) {    echo                         '<a href="', esc_url( add_query_arg( array( 'user_id' => $user->ID, 'wp_http_referer' => urlencode( stripslashes( $_SERVER['REQUEST_URI'] ) ) ), 'user-edit.php' ) ), '"                     >', $user->user_login,                    '</a>';
									...
									<td class="email column-email"><a href="mailto:<?php echo $user->user_email; ?>" title="<?php esc_attr_e( 'E-mail: ', 'register-plus-redux' );
									echo $user->user_email; ?>"><?php echo $user->user_email; ?></a></td>
