...
if ( ! class_exists( 'RPR_Login' ) ) {
	class RPR_Login {
		...
		public /*.void.*/ function rpr_register_form() {
			...
			if ( '1' === $register_plus_redux->rpr_get_option( 'show_disclaimer' ) ) {
				...
				if ( '1' === $register_plus_redux->rpr_get_option( 'require_disclaimer_agree' ) ) {
					echo "\n", '<label id="accept_disclaimer-label" class="accept_check" for="accept_disclaimer"><input type="checkbox" name="accept_disclaimer" id="accept_disclaimer" value="1"';
					if ( ! empty( $accept_disclaimer ) ) { echo ' checked="checked" ';                    }
					if ( 0 !== $tabindex ) { echo 'tabindex="', $tabindex++, '" ';                    }
			...
			if ( '1' === $register_plus_redux->rpr_get_option( 'show_license' ) ) {
				...
				if ( '1' === $register_plus_redux->rpr_get_option( 'require_license_agree' ) ) {
					echo "\n", '<label id="accept_license-label" class="accept_check" for="accept_license"><input type="checkbox" name="accept_license" id="accept_license" value="1"';
					if ( ! empty( $accept_license ) ) { echo ' checked="checked" ';                    }
					if ( 0 !== $tabindex ) { echo 'tabindex="', $tabindex++, '" ';                    }
			...
			if ( '1' === $register_plus_redux->rpr_get_option( 'show_privacy_policy' ) ) {
				...
				if ( '1' === $register_plus_redux->rpr_get_option( 'require_privacy_policy_agree' ) ) {
					echo "\n", '<label id="accept_privacy_policy-label" class="accept_check" for="accept_privacy_policy"><input type="checkbox" name="accept_privacy_policy" id="accept_privacy_policy" value="1"';
					if ( ! empty( $accept_privacy_policy ) ) { echo ' checked="checked" ';                    }
					if ( 0 !== $tabindex ) { echo 'tabindex="', $tabindex++, '" ';                    }
		...
		public /*.void.*/ function rpr_login_head() {
			...
			if ( isset( $_GET['action'] ) && 'register' === $_GET['action'] ) {
						...
						if ( '1' === $meta_field['show_on_registration'] ) {
							...
							if ( 'checkbox' === $meta_field['display'] ) {
								...
								foreach ( $field_options as $field_option ) {
									...
									} else { 										$show_custom_checkbox_fields .= ', #' . $meta_key . '-' . Register_Plus_Redux::sanitize_text( $field_option ) . ', #' . $meta_key . '-' . Register_Plus_Redux::sanitize_text( $field_option ) . '-label';
									}
								}
							}
							if ( 'radio' === $meta_field['display'] ) {
								...
								foreach ( $field_options as $field_option ) {
									...
									} else { 										$show_custom_radio_fields .= ', #' . $meta_key . '-' . Register_Plus_Redux::sanitize_text( $field_option ) . ', #' . $meta_key . '-' . Register_Plus_Redux::sanitize_text( $field_option ) . '-label';
									}
								}
							}
				...
				if ( '1' === $register_plus_redux->rpr_get_option( 'show_disclaimer' ) ) { echo "\n", '#disclaimer { font-size:12px; display: block; width: 100%; padding: 3px; margin-top:2px; margin-right:6px; margin-bottom:8px; background-color:#fff; border:solid 1px #A7A6AA; font-weight:normal;';
					if ( strlen( $register_plus_redux->rpr_get_option( 'message_disclaimer' ) ) > 525 ) { echo 'height: 160px; overflow: auto;';
					} echo ' }'; }
				if ( '1' === $register_plus_redux->rpr_get_option( 'show_license' ) ) { echo "\n", '#license { font-size:12px; display: block; width: 100%; padding: 3px; margin-top:2px; margin-right:6px; margin-bottom:8px; background-color:#fff; border:solid 1px #A7A6AA; font-weight:normal;';
					if ( strlen( $register_plus_redux->rpr_get_option( 'message_license' ) ) > 525 ) { echo 'height: 160px; overflow: auto;';
					} echo ' }'; }
				if ( '1' === $register_plus_redux->rpr_get_option( 'show_privacy_policy' ) ) { echo "\n", '#privacy_policy { font-size:12px; display: block; width: 100%; padding: 3px; margin-top:2px; margin-right:6px; margin-bottom:8px; background-color:#fff; border:solid 1px #A7A6AA; font-weight:normal;';
					if ( strlen( $register_plus_redux->rpr_get_option( 'message_privacy_policy' ) ) > 525 ) { echo 'height: 160px; overflow: auto;';
					} echo ' }'; }
				...
				if ( '1' === $register_plus_redux->rpr_get_option( 'user_set_password' ) && '1' === $register_plus_redux->rpr_get_option( 'show_password_meter' ) ) {
					...
					<script type="text/javascript">
						...
						function check_pass_strength() {
							...
							var user = jQuery("<?php if ( '1' === $register_plus_redux->rpr_get_option( 'username_is_email' ) ) { echo '#user_email';
} else { echo '#user_login';} ?>").val();
		...
		public /*.string.*/ function rpr_filter_login_headertitle( /*.string.*/ $title ) {
			...
			} else { 				$title = get_option( 'blogname' );
			}
			return $title;
		}
	}
